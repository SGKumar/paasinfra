...
public class Java8ComparatorUnitTest {
    ...
    public void whenThenComparing_thenSortedByNameAge() {
        ...
        System.out.println(Arrays.deepToString(sortedEmployeesByAge));

        List<Employee> emps = new ArrayList<>();
        for(Employee emp : sortedEmployeesByAge) {
          emps.add(emp);
        }
        List<String> emps1 = emps.stream().map(Employee::getName).limit(8).collect(Collectors.toList());
        System.out.println(emps1);

        System.out.println("1 sec = " + TimeUnit.SECONDS.toMillis(1));
